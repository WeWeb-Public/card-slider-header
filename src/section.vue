  ...
  <div :style="customStyle">
      ...
      <div class="container-center">
        ...
        >
          ...
          >
            <wwObject
              tag="div"
              v-for="content in feature.contents"
    ...
    >
      <div class="container-center" :style="[mobileStyle, mobileTransition]">
        <div
          class="thumbnail-container"
          v-for="feature in section.data.features"
          :key="feature.uniqueId"
          :style="cardWidth"
        >
          <!-- wwManager:start -->
      ...
      <div class="content-dots-wrapper">
        <li
          ...
          class="content-dot"
          :style="{
            background: sliderPosition == index ? section.data.dotColor : '',
            'border-color': section.data.dotColor,
          }"
          :key="dot.uniqueId"
        >
...
export default {
  ...
  computed: {
    ...
    mobileTransition() {
      return {
        transform:
          "translate(calc(" +
          -this.sliderPosition * (100 / this.featuresLength) +
          "% + " +
          this.sliderPosition * 32 +
          "px ), 0)",
      };
    },
    cardWidth() {
    ...
    columnWidth() {
      return {
        ...
        sm: {
          width: `calc(${100 /
            Math.min(this.section.data.thumbnailsPerLine, 2)}% - 30px)`,
        },
        md: {
          width: `calc(${100 /
            Math.min(this.section.data.thumbnailsPerLine, 3)}% - 30px)`,
        },
        lg: {
          width: `calc(${100 /
            Math.min(this.section.data.thumbnailsPerLine, 4)}% - 30px)`,
        },
      };
    },
  },
  ...
  created() {
    ...
    this.section.data.thumbnailBorderRadius =
      this.section.data.thumbnailBorderRadius || "10px";
          ...
          storyData: {
            fields: [
              ...
              },
              {
                ...
                desc: {
                  en:
                    "Banner border radius: in order top-left | top-right | bottom-right | bottom-left or a single value for all ",
                  fr:
                    "La bordure des coins de la bannière : dans l'ordre en haut à gauche | en haut à droite | en bas à droite | en bas à gauche ou une seule valeur pour tous :",
                },
              },
              {
                label: {
                ...
                desc: {
                  en:
                    "Box-shadow of the banner: offset-x | offset-y | blur-radius | spread-radius | color",
                  fr:
                    "L'ombre de la bannière : offset-x | offset-y | blur-radius | spread-radius | color",
                },
              },
              {
                label: {
